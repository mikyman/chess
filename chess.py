...
def checkShortCastleRight(color,moveHistory) -> bool:
  ...
  if color == "White":
    if board.loc["1","e"] == "wk" and board.loc["1","f"] == "--" and board.loc["1","g"] == "--" \
      and board.loc["1","h"] == "wr":
  ...
  if color == "Black":
    if board.loc["8","e"] == "bk" and board.loc["8","f"] == "--" and \
      board.loc["8","g"] == "--":
        if board.loc["8","h"] == "br":
          for move in moveHistory:
...
def checkLongCastleRight(color,moveHistory) -> bool:
  ...
  if color == "White":
    if board.loc["1","a"] == "wr" and board.loc["1","b"] == "--" and \
      board.loc["1","c"] == "--":
        if board.loc["1","d"] == "--" and board.loc["1","e"] == "wk":
          for move in moveHistory:
  ...
  if color == "Black":
    if board.loc["8","a"] == "br" and board.loc["8","b"] == "--" and \
      board.loc["8","c"] == "--":
        if board.loc["8","d"] == "--" and board.loc["8","e"] == "bk":
          for move in moveHistory:
...
def move(piece,shortCastleRight,longCastleRight,sourceSquareRank,sourceSquareLine,targetSquareRank,\
        targetSquareLine,capturableEnPassant) -> bool:

  hasTakenPiece = False

  # kurze Rochade weiß
  if piece == "wk" and shortCastleRight and (targetSquareLine in [6,7]):
    board.iloc[7,4] = "--"
    board.iloc[7,6] = "wk"
    board.iloc[7,7] = "--"
    board.iloc[7,5] = "wr"
  ...
  # lange Rochade weiß
  elif piece == "wk" and longCastleRight and (targetSquareLine in [0,2]):
    board.iloc[7,4] = "--"
    board.iloc[7,2] = "wk"
    board.iloc[7,0] = "--"
    board.iloc[7,3] = "wr"
  ...
  # kurze Rochade schwarz
  elif piece == "bk" and shortCastleRight and (targetSquareLine in [6,7]):
    board.iloc[0,4] = "--"
    board.iloc[0,6] = "bk"
    board.iloc[0,7] = "--"
    board.iloc[0,5] = "br"
  ...
  # lange Rochade schwarz
  elif piece == "bk" and longCastleRight and (targetSquareLine in [0,2]):
    board.iloc[0,4] = "--"
    board.iloc[0,2] = "bk"
    board.iloc[0,0] = "--"
    board.iloc[0,3] = "br"
...
def isUnderAttack(color,mySquareRank,mySquareLine) -> bool:
  ...
  # Bauer
  # -- EDIT -- set function call 'color[0].lower()' to var 'selfColor' and
  # replaced it all in this function
  selfColor = color[0].lower()
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "p"):
      return True

  # ASK: need list as argument really??
  # Turm
  piece = selfColor+"r"
  dangerFields = []
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "r"):
      return True
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "b"):
      return True
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "q"):
      return True
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "n"):
      return True
  ...
  for df in dangerFields:
    if board.iloc[df[0],df[1]] == (otherColor[selfColor] + "k"):
      return True
...
def startGame():
  ...
  while True:
    ...
    while True:
      if (playerWhite == "human" and color == "White") or (playerBlack == "human" and color == "Black"):
        ...
        if len(sourceSquare) != 2 or sourceSquare[0] not in letters or sourceSquare[1] not in numbers:
          continue
      elif (playerWhite == "engine" and color == "White") or (playerBlack == "engine" and color == "Black"):
      ...
      legalMovesV1 = []
      legalMovesV1 = checkLegalMovesV1(color,piece,legalMovesV1,sourceSquareRank,sourceSquareLine,\
                                      capturableEnPassant) 
      legalMovesV1,shortCastleRight,longCastleRight = checkCastleMovesV1(color, isMyKingInCheck,piece,\
                                                                        legalMovesV1,moveHistory)
      legalMovesV3 = []
      legalMovesV2 = checkLegalMovesV2(color,piece,legalMovesV1,legalMovesV3,sourceSquareRank,\
                                      sourceSquareLine)
    ...
    while True:
      ...
      hasTakenPiece     = move(piece,shortCastleRight,longCastleRight,sourceSquareRank,sourceSquareLine,\
                              targetSquareRank,targetSquareLine,capturableEnPassant)
...
if __name__=="__main__":
  startGame()
  # feedback: macht unaufgefordert rochade in einen doppelzug
  '''
  White to move:
  From: c3
  To: d4
  ...
  4  --  --  --  (wp)  --  --  --  -- # right move
  3  --  --  (--)  --  --  --  --  --
  2  wp  wp  --  wn  wp  wp  wp  wp
  1  wr  --  --  --  wk  wb  --  wr

  Black to move: # played for white player??
  From: e1
  To: c1
